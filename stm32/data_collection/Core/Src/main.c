...
int main(void)
{
  ...
  while (1)
  {
		...
		else if (idx == HEAT_ON+1){
			heater = 0;
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET);
		}
		else if (idx == HEAT_OFF+HEAT_ON+1){
			idx = 0;
		}
		idx++;
		...
		if (ret != HAL_OK) {
			...
			if (ret != HAL_OK) {
				...
				log_info("%lu %.2f %d",(uint32_t)prev_time,temp_c, heater);
			}
		}
		while(HAL_GetTick()-prev_time < 20);
    /* USER CODE END WHILE */
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
